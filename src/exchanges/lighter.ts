...
const PAIR_TO_BASE: Record<string, string> = {
  ...
  ETH: 'ETH',
  SOL: 'SOL',
  XAUT: 'XAUT', // May not exist on Lighter
};
...
export class LighterAdapter implements ExchangeAdapter {
  ...
  private async loadMeta(): Promise<void> {
    ...
    try {
      ...
      const books: any[] = data.order_books ?? [];
      for (const book of books) {
        // Only include perp markets (symbol is just "BTC", "ETH", etc.)
      ...
      // Leave metaLoaded false so next call retries
    }
  }
  ...
  async fetchOrderbook(pair: string, limit: number): Promise<Orderbook | null> {
    ...
    try {
      const cappedLimit = Math.min(limit, 200); // Lighter API max is 200
      ...
      const asks: OrderbookEntry[] = rawAsks.map((o: any) => ({
        price: parseFloat(o.price),
        amount: parseFloat(o.remaining_base_amount),
      })).sort((a, b) => a.price - b.price);

      const bids: OrderbookEntry[] = rawBids.map((o: any) => ({
        price: parseFloat(o.price),
        amount: parseFloat(o.remaining_base_amount),
